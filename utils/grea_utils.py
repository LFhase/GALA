...
def get_args():
    ...
    args.path_list = eval(args.path_list)
    return args

cls_criterion = torch.nn.BCEWithLogitsLoss()
reg_criterion = torch.nn.MSELoss()
def train(args, model, device, loader, optimizers, task_type, optimizer_name, epoch=0):
    if args.dataset.lower() in ['spmotif', 'mspmotif', 'pspmotif','bspmotif']:
...
def eval_model(args, model, device, loader, evaluator):
    model.eval()
    y_true = []
    y_pred = []
