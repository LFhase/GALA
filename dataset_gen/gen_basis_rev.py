...
for _ in tqdm(range(num_train_perc)):
    base_num = 1
    motif_id = np.random.choice([0, 1, 2], p=[(1 - bias) / 2, bias, (1 - bias) / 2])
...
for _ in tqdm(range(num_train_perc)):
    base_num = 2
    motif_id = np.random.choice([0, 1, 2], p=[bias, (1 - bias) / 2, (1 - bias) / 2])
    if random.random() < label_noise:
        base_num = random.randint(1, 3)
    if base_num == 1:
        base = 'tree'
...
for _ in tqdm(range(num_train_perc)):
    base_num=3
    motif_id = np.random.choice([0, 1, 2], p=[(1 - bias) / 2, (1 - bias) / 2, bias])
    if random.random() < label_noise:
        base_num = random.randint(1, 3)
    if base_num == 1:
        base = 'tree'
...
print(np.mean(n_mean), np.mean(e_mean))
print(len(ground_truth_list))
if not os.path.exists(f'../data/rSPMotif-{global_b}/'):
    os.mkdir(f'../data/rSPMotif-{global_b}/')
if not os.path.exists(f'../data/rSPMotif-{global_b}/raw'):
    os.mkdir(f'../data/rSPMotif-{global_b}/raw')
np.save(f'../data/rSPMotif-{global_b}/raw/train.npy',
...
print(np.mean(n_mean), np.mean(e_mean))
print(len(ground_truth_list))
np.save(f'../data/rSPMotif-{global_b}/raw/val.npy',
        (edge_index_list, label_list, base_list,ground_truth_list, role_id_list, pos_list, motif_id_list))
...
print(np.mean(n_mean), np.mean(e_mean))
print(len(ground_truth_list))
np.save(f'../data/rSPMotif-{global_b}/raw/test.npy',
        (edge_index_list, label_list, base_list,ground_truth_list, role_id_list, pos_list, motif_id_list))
